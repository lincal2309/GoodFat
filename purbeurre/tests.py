...
class TestProductDetail(TestCase):
    def setUp(self):
        ...
        Product.objects.create(id=1, code=1, name="Produit 1", description="Description du produit 1",
            ingredients="Ingrédients du produit 1", nutrition_score = 1, nutrition_grade="a",
            img_url="http://img.produit1.off.fr/1", off_url="http://code.produit1.off.fr/1",
            brand='Badoit', category=c1)

    @mock.patch('wikipediaapi.Wikipedia')
    def test_display_product_detail_no_wiki(self, mock_get_wiki):
        # Check product's detail display :
        #   - Case API find no information

        expected_result = ""

        mock_get_wiki.return_value.page.return_value.text = expected_result

        response = self.client.get('/substitute/1')
        self.assertEqual(response.context["product"].name, 'Produit 1')
        self.assertEqual(response.context["product"].brand, 'Badoit')
        self.assertHTMLEqual(response.context["wiki_snippet"], "")
        self.assertContains(response, "pas encore de page dédiée sur Wikipedia")

    @mock.patch('wikipediaapi.Wikipedia')
    def test_display_product_detail_short_wiki(self, mock_get_wiki):
        # Check product's detail display :
        #   - Case API find short informatino (page with one single section)

        expected_result = '''<body>
                <div class="container align-items-center">
                    <div class="row align-items-center">
                        <p>Un texte présentant la marque de mon produit</p>
                    </div>
                </div>
            </body>'''
    ...
    @mock.patch('wikipediaapi.Wikipedia')
    def test_display_product_detail_long_wiki(self, mock_get_wiki):
        # Check product's detail display :
        #   - Case API find short informatino (page with more than one section)

        expected_result = '''<body>
            <div class="row align-items-center">
                <p>Un texte présentant la marque de mon produit</p>
            </div>
            <h2>Une section avec des détails</h2>
            <div class="row align-items-center">
                    <p>Du texte pour décrire les informations détaillées</p>
            </div>
        </body>'''
